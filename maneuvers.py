...
class Maneuvers:
  ...
  def betts(self,y,t):
    ...
    if self.useSecularCoe:
      TOrbit = 2*np.pi*(self.history.coe[-1,0]**3/constants.mu_E)**0.5
      tPartition = TOrbit
      idxf = np.abs(self.history.t - tPartition).argmin();

      coe[0:3] = np.mean(self.history.coe[-idxf:,0:3],axis=0)
      coe[4] = np.mean(self.history.coe[-idxf:,4],axis=0)
  ...
  def propagate(self,time,timestep):
    ...
    while True:
      if self.targetRun and self.useSecularCoe:
        # self.history.t and self.history.coe change on every loop and therefore tPartition changes accordingly
        TOrbit = 2*np.pi*(self.history.coe[-1,0]**3/constants.mu_E)**0.5
        tPartition = self.history.t[-1] + TOrbit

        if tPartition > t[-1]:
          tInt = t[idx0:]
        else:
          idxf = np.abs(t - tPartition).argmin();
          #print(idx0,idxf)
          tInt = t[idx0:idxf+1]
        #print("tInt:\n",tInt)
      else:
  ...
  def setTargetRun(self,coe,**kwargs):
    for key in kwargs:
      if key == "use_secular_coe":
        self.useSecularCoe = kwargs[key];

    self.targetRun = True
    self.targetOrbit = coe
